...
export async function openDetails(p) {
        ...
        <div class="p-4">
            ...
            <div class="flex flex-row gap-3 mb-4 bg-gray-800/40 p-3 rounded-2xl border border-white/5">
                ...
                <div class="flex-1 flex flex-col justify-between min-w-0 gap-2">
                    ...
                    <!-- Datos físicos: 3 en una fila -->
                    <div class="flex gap-1 text-[9px] font-bold uppercase">
                        <div class="flex-1 bg-black/30 px-2 py-1 rounded-lg">
                            <span class="text-gray-500 block text-[7px]">Altura</span>
                            ${p.física?.altura} m
                        </div>
                        <div class="flex-1 bg-black/30 px-2 py-1 rounded-lg">
                            <span class="text-gray-500 block text-[7px]">Peso</span>
                            ${p.física?.peso} kg
                        </div>
                        <div class="flex-1 bg-black/30 px-2 py-1 rounded-lg border border-green-500/20 text-green-400">
                            <span class="text-gray-500 block text-[7px]">H. Lazo</span>
                            ${gkPower} pw
                        </div>
                    </div>
    ...
    // Detectar fondo negro en sprite del panel de detalles
    ...
    content.querySelectorAll('#moves-tabs button').forEach(btn => {
        btn.addEventListener('click', () => {
            content.querySelectorAll('#moves-tabs button').forEach(b => b.classList.remove('active-tab'));
            btn.classList.add('active-tab');
            document.getElementById('moves-container').innerHTML = renderMovesList(p, btn.dataset.tab);
        });
    });
    // Activar tab inicial
    content.querySelector('#moves-tabs button')?.classList.add('active-tab');

    panel.classList.add('open');
    if (window.innerWidth >= 1024) {
        mainLayout.style.marginRight = "40%";
        // Bajar a 4 columnas mientras el panel está abierto
